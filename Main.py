...
async def displaySearch(ctx):
    ...
    embed = discord.Embed(
        ...
        description = 'Here is your search output!'
    )
    embed.set_footer(text='Please contact an Admin if you do not want to see these messages')
    embed.set_image(url='https://cdn5-ss1.sharpschool.com/UserFiles/Servers/Server_4232405/Image/Central%20Office/jobhunt.jpg')
...
async def sendJob(ctx, user: discord.Member = None):
    #await user.send(bot.links[1])
    #await user.send(bot.links[2])
    embed = discord.Embed(
        title = 'Here is a job for you!',
...
bot.run('OTU1MTU3NzgzOTk5ODM2MTYw.YjdmAQ.9PqePLorCqrB_fI4RA3tzhUoHX4')